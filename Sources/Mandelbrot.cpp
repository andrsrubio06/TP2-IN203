...
std::vector<int>
computeMandelbrotSet( int W, int H, int maxIter, int H_local, int H_ctrl)
{
    std::chrono::time_point<std::chrono::system_clock> start, end;
    std::vector<int> pixels(W*H_local);
    start = std::chrono::system_clock::now();
    // On parcourt les pixels de l'espace image :
    for ( int i = 0; i < H_local; ++i ) {
      computeMandelbrotSetRow(W, H, maxIter, H_ctrl + i, pixels.data() + W*(H_local-i-1) );
    }
...
int main(int argc, char *argv[] ) 
 { 
    ...
    int H_local, H_ctrl_ligne;
    H_local = H/nbp;
    H_ctrl_ligne = rank*H_local;

    auto iters = computeMandelbrotSet( W, H, maxIter, H_local, H_ctrl_ligne);
    vector<int> picture;
    ...
    MPI_Gather(iters.data(), H_local * W, MPI_INT, picture.data(), H_local * W, MPI_INT, 0, MPI_COMM_WORLD);
